			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="serieName" title="${message(code: 'serie.serieName.label', default: 'Serie Name')}" />
					
						<g:sortableColumn property="place" title="${message(code: 'serie.place.label', default: 'Place')}" />
				...
				<g:each in="${serieInstanceList}" status="i" var="serieInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${serieInstance.id}">${fieldValue(bean: serieInstance, field: "serieName")}</g:link></td>
					
						<td>${fieldValue(bean: serieInstance, field: "place")}</td>
